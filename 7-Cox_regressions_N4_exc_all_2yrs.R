...
#-----------------------------------------------------------------------------#
#                          Remove first 2 years of FU                          #
#-----------------------------------------------------------------------------#

data$Age_Rec_Exit_Diff <- data$Agexit_Stom - data$Age_Recr

#Remove participants censored during first 2 years of FU
data <- data[which(!(data$Age_Rec_Exit_Diff <= 2)),]

#Remove cases diagnosed during first 2 years of FU 
#data <- data[which(!(data$Age_Rec_Exit_Diff <= 2 & (data$Esoph_Mal_Adeno == 1 | data$HN_Mal_vitd_SiteCode == 1))),]
#####we didn't use this in the paper, but checked to see if it made any difference 
#####Results were consistent with those obtained in the main analysis
