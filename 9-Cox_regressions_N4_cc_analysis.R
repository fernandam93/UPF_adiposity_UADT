...
data <- data[!is.na(data$Smoke_Stat) & !is.na(data$L_School) & !is.na(data$Pa_Index),]
...
cox_func <- function(ageexit, outcome, exposure, exposure_var, outcome_var) {
  ...
  x2 <- survival::coxph(Surv(Age_Recr, ageexit, outcome) ~ exposure + strata(Sex, Cntr_C, Age_Recr_1y) + factor(Smoke_Stat) +
                          factor(L_School) + factor(Pa_Index) + Height_C, data = data)
  #write function to run cox regression model 3
  x3 <- survival::coxph(Surv(Age_Recr, ageexit, outcome) ~ exposure + strata(Sex, Cntr_C, Age_Recr_1y) + factor(Smoke_Stat) +
                          factor(L_School) + factor(Pa_Index) + Height_C + Alc_Re, data = data)
...
cox_Larynx_Mal_INHANCE_2007_SiteCode_table <- cox_func(data$Agexit_Uadt, data$Larynx_Mal_INHANCE_2007_SiteCode, data$QE_P_M_N4, "UPF intake in %g/d", "Larynx cancer (INHANCE_2007)")
...
#create table for head and neck cancer cox results 
HN_subtypes_Mal_table <- rbind(cox_Larynx_Mal_INHANCE_2007_SiteCode_table, cox_Oral_Mal_INHANCE_2007_table, cox_Oropharynx_Mal_INHANCE_2007_SiteCode_table,
