...
#create dataframe with cox regression results
cox_table_func_g <- function(cox_object, model_num, exposure_var, outcome_var) {
  x <- tidy(cox_object) %>% 
    filter(., row_number()==1) %>%
...
cox_func_g <- function(ageexit, outcome, exposure, exposure_var, outcome_var) {
  #write function to run cox regression model 1
  x1 <- survival::coxph(Surv(Age_Recr, ageexit, outcome) ~ exposure + strata(Sex, Cntr_C, Age_Recr_1y), data = data)
  #write function to run cox regression model 2
  x2 <- survival::coxph(Surv(Age_Recr, ageexit, outcome) ~ exposure + strata(Sex, Cntr_C, Age_Recr_1y) + factor(Smoke_Stat_C) +
  ...
  cox_table1 <- cox_table_func_g(x1, 1, exposure_var, outcome_var)
  cox_table2 <- cox_table_func_g(x2, 2, exposure_var, outcome_var)
  cox_table3 <- cox_table_func_g(x3, 3, exposure_var, outcome_var)
...
cox_Esoph_Mal_Adeno_table_grams <- cox_func_g(data$Agexit_Stom, data$Esoph_Mal_Adeno, data$QE_M_N4, "UPF intake in g/d", "Oesophageal adenocarcinoma")

#create table for oesophageal cancer cox results 
Esoph_Mal_morp_table_grams <- cox_Esoph_Mal_Adeno_table_grams %>% flextable(.)

cox_HN_Mal_INHANCE_2007_SiteCode_table_grams <- cox_func_g(data$Agexit_Uadt, data$HN_Mal_INHANCE_2007_SiteCode, data$QE_M_N4, "UPF intake in g/d", "Head and neck cancer (INHANCE_2007)")

#create table for head and neck cancer cox results 
HN_Mal_table_grams <- cox_HN_Mal_INHANCE_2007_SiteCode_table_grams %>% flextable(.)


cox_Larynx_Mal_INHANCE_2007_table_grams <- cox_func_g(data$Agexit_Uadt, data$Larynx_Mal_INHANCE_2007_SiteCode, data$QE_M_N4, "UPF intake in g/d", "Larynx cancer (INHANCE_2007)")
cox_Oral_Mal_INHANCE_2007_table_grams <- cox_func_g(data$Agexit_Uadt, data$Oral_cavity_Mal_INHANCE_2007_SiteCode, data$QE_M_N4, "UPF intake in g/d", "Oral cancer (INHANCE_2007)")
cox_Oropharynx_Mal_INHANCE_2007_SiteCode_table_grams <- cox_func_g(data$Agexit_Uadt, data$Oropharynx_Mal_INHANCE_2007_SiteCode, data$QE_M_N4, "UPF intake in g/d", "Oropharynx cancer (INHANCE_2007)")
cox_Hypopharynx_Mal_INHANCE_2007_SiteCode_table_grams <- cox_func_g(data$Agexit_Uadt, data$Hypopharynx_Mal_INHANCE_2007_SiteCode, data$QE_M_N4, "UPF intake in g/d", "Hypopharynx cancer (INHANCE_2007)")
cox_Unspecified_Mal_INHANCE_2007_SiteCode_table_grams <- cox_func_g(data$Agexit_Uadt, data$Unspecified_Mal_INHANCE_2007_SiteCode, data$QE_M_N4, "UPF intake in g/d", "Unspecified/overlapping cancer (INHANCE_2007)")

#create table for head and neck cancer cox results 
HN_subtypes_Mal_table_grams <- rbind(cox_Larynx_Mal_INHANCE_2007_table_grams, cox_Oral_Mal_INHANCE_2007_table_grams, cox_Oropharynx_Mal_INHANCE_2007_SiteCode_table_grams,
                               cox_Hypopharynx_Mal_INHANCE_2007_SiteCode_table_grams, cox_Unspecified_Mal_INHANCE_2007_SiteCode_table_grams) %>% flextable(.)
...
#save g/d tables
Esoph_Mal_morp_table_grams %>% 
  save_as_docx(path="FILE.docx", pr_section = sect_properties)

HN_Mal_table_grams %>% 
  save_as_docx(path="FILE.docx", pr_section = sect_properties)

HN_subtypes_Mal_table_grams %>% 
  save_as_docx(path="FILE.docx", pr_section = sect_properties)
...
############ export to excel file grams
cox_func_excel_grams <- function(cox_object, model_num, exposure_var, outcome_var) {
  x <- tidy(cox_object) %>% 
    filter(., row_number()==1) %>%
...
excel_func_grams <- function(ageexit, outcome, exposure, exposure_var, outcome_var) {
  #write function to run cox regression model 1
  x1 <- survival::coxph(Surv(Age_Recr, ageexit, outcome) ~ exposure + strata(Sex, Cntr_C, Age_Recr_1y), data = data)
  #write function to run cox regression model 2
  x2 <- survival::coxph(Surv(Age_Recr, ageexit, outcome) ~ exposure + strata(Sex, Cntr_C, Age_Recr_1y) + factor(Smoke_Stat_C) +
  ...
  cox_table1 <- cox_func_excel_grams(x1, 1, exposure_var, outcome_var)
  cox_table2 <- cox_func_excel_grams(x2, 2, exposure_var, outcome_var)
  cox_table3 <- cox_func_excel_grams(x3, 3, exposure_var, outcome_var)
...
#HNC and OAC
cox_HN_Mal_excel_grams <- excel_func_grams(data$Agexit_Uadt, data$HN_Mal_INHANCE_2007_SiteCode, data$QE_M_N4, "Ultra-processed foods", "Head and neck cancer")
cox_Esoph_Mal_Adeno_excel_grams <- excel_func_grams(data$Agexit_Stom, data$Esoph_Mal_Adeno, data$QE_M_N4, "Ultra-processed foods", "Oesophageal adenocarcinoma")


HN_O_Mal_grams_excel <- rbind(cox_HN_Mal_excel_grams, 
                              cox_Esoph_Mal_Adeno_excel_grams)

write.xlsx(HN_O_Mal_grams_excel, file = "FILE.xlsx")
